...
Journal processing state is tracked via HTML comment markers embedded in journal files (`<!-- normalized: ... -->`, `<!-- fences-verified: ... -->`). This is fundamentally broken because journal files contain AI conversations that include these exact strings as content, causing false positives. Similarly, `countUnenriched()` scans file content for `---` prefix, but section breaks also use `---`. The user is okay nuking all journals and re-importing from JSONL.
...
1. **Create `internal/journal/state/state.go`** — types (`JournalState`, `FileState`), `Load`/`Save` (atomic write via temp+rename), query helpers (`IsEnriched`, `CountUnenriched`), mutation helpers (`MarkImported`, `MarkEnriched`, `Rename`).
...
### Phase 2: Wire into import

4. **`internal/cli/recall/run.go`** — In `runRecallImport()`, load state at start, call `MarkImported()` after each file write, `Rename()` when slug changes, `Save()` at end.
...
2. `ctx recall import --all` into empty journal dir → `.state.json` created with imported dates
