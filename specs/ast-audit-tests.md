...
| `TestNoRawPermissions` | Octal file permission literals (`0644`, `0755`, `0600`, `0700`, `0750`) must not appear outside `internal/config/fs/`. All other packages (including `internal/io/`) must use `config/fs` constants (`fs.PermFile`, `fs.PermExec`, `fs.PermSecret`, etc.). Test files exempt. |
