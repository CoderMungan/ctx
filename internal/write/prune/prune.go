...
// DryRunLine prints a single dry-run prune candidate.
//
// Parameters:
//   - cmd: Cobra command for output. Nil is a no-op.
//   - name: file name being considered for pruning.
//   - age: human-readable age string.
func DryRunLine(cmd *cobra.Command, name, age string) {
	if cmd == nil {
		return
	}
...
// ErrorLine prints an error encountered while removing a file.
//
// Parameters:
//   - cmd: Cobra command for error output. Nil is a no-op.
//   - name: file name that failed to remove.
//   - err: the removal error.
func ErrorLine(cmd *cobra.Command, name string, err error) {
	if cmd == nil {
		return
	}
...
// Summary prints the prune results summary.
//
// Parameters:
//   - cmd: Cobra command for output. Nil is a no-op.
...
func Summary(cmd *cobra.Command, dryRun bool, pruned, skipped, preserved int) {
	if cmd == nil {
		return
	}
	if dryRun {
