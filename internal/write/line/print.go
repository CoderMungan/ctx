...
// Count prints a formatted count line if count is positive.
...
func Count(cmd *cobra.Command, key string, count int) {
	if cmd == nil || count <= 0 {
		return
	}
...
// All prints each line to cmd's output. Nil cmd is a no-op.
//
...
func All(cmd *cobra.Command, lines []string) {
	if cmd == nil {
		return
	}
