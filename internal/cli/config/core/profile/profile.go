...
// Detect returns the active profile name from the parsed
// .ctxrc. Returns "" if .ctxrc is missing or has no profile
// field.
//
// Returns:
...
// Copy copies a source profile file to .ctxrc.
//
// Parameters:
...
func Copy(root, srcFile string) error {
...
// SwitchTo copies the requested profile to .ctxrc and
// returns a status message.
//
// If the requested profile is already active, returns a
// no-op message. If .ctxrc did not previously exist, returns
// a "created" message.
//
// Parameters:
//   - root: Git repository root directory
...
func SwitchTo(root, profile string) (string, error) {
	...
	if current == profile {
		return fmt.Sprintf(
			desc.Text(text.DescKeyConfigAlreadyOn),
			profile,
		), nil
	}
	...
	if current == "" {
		return fmt.Sprintf(
			desc.Text(text.DescKeyConfigCreated),
			file.CtxRC, profile,
		), nil
	}
	return fmt.Sprintf(
		desc.Text(text.DescKeyConfigSwitched), profile,
	), nil
}

// GitRoot returns the git repository root directory.
//
// Returns an error if git is not installed or the current
// directory is not inside a git repository. Features that
// depend on git should degrade gracefully when this returns
// an error.
//
// Returns:
