...
// CompactTasks moves completed tasks to the "Completed"
// section in TASKS.md.
//
// Scans TASKS.md for checked items ("- [x]") outside the
// Completed section, including their nested content (indented
// lines below the task). This only moves tasks where all
// nested subtasks are also complete. Optionally archives
// them to .context/archive/.
//
// Parameters:
//   - cmd: Cobra command for output messages
//   - ctx: Loaded context containing the files
//   - archive: If true, write completed tasks to a dated
//     archive file
//
// Returns:
...
func CompactTasks(
	...
	for _, taskText := range result.TasksMoved {
		compact.InfoMovingTask(
			cmd, tidy.TruncateString(taskText, token.TruncateLen),
		)
	}
	for _, taskText := range result.TasksSkipped {
		truncated := tidy.TruncateString(
			taskText, token.TruncateLen,
		)
