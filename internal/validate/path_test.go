	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Boundary(%q) error = %v, wantErr %v",
...
func TestBoundaryCaseInsensitive(t *testing.T) {
	...
	if err := Boundary(".context"); err != nil {
		t.Errorf("Boundary(.context) with case-mangled cwd: %v", err)
	}
}
...
func TestBoundary_WindowsCaseInsensitive(t *testing.T) {
	...
	if len(cwd) >= 2 && cwd[1] == ':' {
		...
		if err := Boundary(nonExistent); err != nil {
			t.Errorf("Boundary(%q) with swapped drive case should pass, got: %v", nonExistent, err)
		}

		// Also test the default relative path that ctx init uses
		if err := Boundary(".context"); err != nil {
			t.Errorf("Boundary(.context) with swapped drive case should pass, got: %v", err)
		}
	}
}
