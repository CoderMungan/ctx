...
For three releases, ctx required assembly. Clone the repo. Build the
binary. Copy hook scripts into `.claude/hooks/`. Symlink skill files.
Understand which shell scripts called which Go commands. Hope nothing
broke when Claude Code updated its hook format.
...
distribution a solved problem. When you fix how something ships, you
end up fixing how it is built.

!!! info "The Release Window"
    **February 15 -- February 16, 2026**

    From the v0.3.0 tag to commit `a3178bc`.
    109 commits. 334 files changed. Version jumped from 0.3.0 to 0.6.0
    to signal the magnitude.
...
Skills are served directly from `internal/tpl/claude/skills/`; there
...
That last point matters. Before v0.6.0, `ctx init` tried to set up
Claude Code integration as part of initialization. That coupled the
context system to a specific tool. Now, `ctx init` gives you persistent
context. The plugin gives you Claude Code integration. They compose;
they don't depend.
...
AES-256-GCM encrypted storage for sensitive one-liners. The encrypted
blob commits to git; the key stays in `.gitignore`. Useful for
connection strings, API keys, and other values that need to travel
with the project without appearing in plaintext.

### Security Hardening

Three medium-severity findings from a security audit, now closed:
...
Obsidian export logic, and the scratchpad encryption module. The core
CLI grew modestly; the ecosystem around it grew substantially.
