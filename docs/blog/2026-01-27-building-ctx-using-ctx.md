...
1. Create a `.context/` directory with structured 
   Markdown files for decisions, learnings, tasks, and conventions. 
2. The AI reads these at session start and writes to them before 
   the session ends.
...
This session file (*written by the AI to preserve its own context*)
became the **template** for how `ctx` handles session persistence.

## The Rename

By January 21st, I realized "*Active Memory*" was too generic, 
and (*arguably*) too marketing-smelly. 
...
So I took a mental note to schedule 
**[periodic consolidation sessions](../blog/2026-02-17-the-3-1-ratio.md)**.
...
By the time of this writing this project's `ctx` sessions 
(`.context/sessions/`) contains 40+ files from this project's development.
...
!!! note "Update"
    ...
    In current releases, `ctx` uses a **journal** instead: the enrichment 
    process generates meaningful slugs from context automatically, so there 
    is no need to manually save sessions.

The `SessionEnd` hook captured transcripts automatically.
Even `Ctrl+C` was caught.
...
But I had to schedule 
**[consolidation sessions](../blog/2026-02-17-the-3-1-ratio.md)**.
...
It's not about loading them into context: It is about having them 
**when** you need them.
